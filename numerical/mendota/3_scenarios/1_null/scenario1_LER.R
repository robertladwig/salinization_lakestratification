...
inflow.df$Salinity_practicalSalinityUnits <- 0.0
months <- month(inflow.df$datetime)
idx <- which(months %in% c(12, 1, 2, 3, 4))
# inflow.df$Salinity_practicalSalinityUnits[idx] <- 2.0
...
file.copy(from = '../salt_cond_null.dat', to = 'GOTM/', overwrite = T)
input_yaml_multiple(file = 'GOTM/gotm.yaml', value = "salt_cond_null.dat",
