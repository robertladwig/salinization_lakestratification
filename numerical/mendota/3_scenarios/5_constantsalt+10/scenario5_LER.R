...
inflow.df$Salinity_practicalSalinityUnits[idx] <- 10.0
...
# 
# df.meteo <- read.csv('LakeEnsemblR_meteo_standard.csv')
# wind <- df.meteo %>%
#    dplyr::filter(datetime >= '1995-01-01 00:00:00' & datetime <= '2015-12-30 00:00:00') %>%
#    dplyr::mutate(date = as.Date(datetime)) %>%
#    dplyr::group_by(date) %>%
#    dplyr::summarise(mean(Ten_Meter_Elevation_Wind_Speed_meterPerSecond))
# 
# ts.ln <- lapply(out, function(x) {
#    ts.lake.number(x, wnd = wind, wnd.height = 10, bathy = bathy, seasonal = TRUE)
# })
# ## Reshape to data.frame
# df <- reshape2::melt(ts.sch, id.vars = 1)
# colnames(df)[4] <- "model"
